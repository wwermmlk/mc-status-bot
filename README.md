...
- **디스코드 → 게임** — 게임 채팅에 <code>[Discord]</code>(디스코드 브랜드 색 `#5865F2`) `닉네임: 메시지`로 표시. 서버가 꺼져 있으면 ❌ 반응으로 실패 안내
  - 디스코드 서버 소유자의 닉네임은 **금색**으로 표시 (닉네임이 아닌 계정 ID로 판별해 흉내 불가)
  - 전달되면 게임에 실제로 표시된 내용(잘림·이모지 변환 포함)을 #chat에 한 줄로 다시 보여줌
...
### 일부 플레이어만 스티브 얼굴로 나온 문제

채팅 아바타가 일부 플레이어만 기본 스티브 얼굴로 표시됐습니다. 해당 플레이어들을 Mojang API로 조회하니 모두 정품 계정이었고
세션 서버에 **커스텀 스킨이 정상 등록**돼 있었습니다. 같은 UUID로 여러 스킨 이미지 서비스를 비교한 결과,
기존에 쓰던 mc-heads.net만 이들에게 오류 대신 **기본 얼굴 이미지를 정상 응답(200)으로** 돌려주고 있었습니다.
이미지를 받아 "존재하지 않는 계정"의 기본 얼굴과 해시를 비교해 판별했고, 이름 조회로도 전원 정상이던 minotar로 교체했습니다.
외부 서비스는 또 바뀔 수 있어 `AVATAR_URL`로 교체할 수 있게 했습니다.

교체 뒤에도 "아직 스티브가 보인다"는 제보가 있었는데, 디스코드가 웹훅 메시지마다 붙이는 아바타 이미지 키를 모아 보니
스티브 키를 여러 플레이어가 공유한 것은 **교체 이전 메시지(9건)뿐**이었고 교체 이후 113건은 모두 서로 다른 이미지였습니다.
웹훅 메시지의 아바타는 게시 시점에 고정되어 과거 메시지는 바뀌지 않기 때문이었습니다.
...
상시 실행은 Windows 작업 스케줄러에 **창 없이** 등록합니다.

| 항목 | 값 |
|---|---|
| 트리거 | 로그온할 때 |
| 프로그램 | `pythonw.exe` (Python 설치 폴더, 콘솔 창 없음) |
| 인수 / 시작 위치 | `bridge.py` / 저장소 폴더 |

- 실행 기록은 `bridge.log`에 남습니다(1MB씩 최대 4개 순환).
- 브리지는 로컬 포트 하나를 잠금으로 잡아 **중복 실행을 막습니다.** 두 개가 돌면 모든 채팅이 두 번씩 게시되기 때문입니다.
...
| `AVATAR_URL` | | 채팅 아바타 이미지 주소(`{name}` 치환), 기본 minotar |
| `OWNER_NAME_COLOR` | | 서버 소유자 닉네임 색(`#RRGGBB`), 기본 `#FFAA00` |
